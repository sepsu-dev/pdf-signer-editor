...
export default function Toolbar({
  ...
  return (
    <header className="sticky top-0 z-30 w-full bg-white/95 backdrop-blur-md border-b border-slate-200/80 shadow-xs px-4 py-2 flex items-center justify-between gap-3">
      ...
      <div className="flex items-center gap-3 min-w-0">
        ...
        >
          ...
          <span className="font-bold text-sm tracking-tight text-slate-800">
            PDF <span className="text-rose-500">Signer Editor</span>
          </span>
        </div>

        {/* File Name Chip with Rename & Close */}
        {fileName && (
          <div className="group/file flex items-center gap-1.5 pl-2.5 pr-1.5 py-1 rounded-2xl bg-rose-50/80 hover:bg-rose-100/70 border border-rose-200/80 text-rose-950 text-xs transition-colors max-w-[200px] sm:max-w-[300px]">
      ...
      {/* Center: Pagination & Zoom (When PDF is loaded) */}
      {hasFile && (
      ...
      {/* Right: Clean & Simple Action Tools */}
      <div className="flex items-center gap-2 shrink-0">
        <button
          type="button"
          disabled={!hasFile}
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3.5 py-2 rounded-2xl bg-rose-600 hover:bg-rose-700 text-white text-xs font-semibold shadow-xs disabled:opacity-40 disabled:cursor-not-allowed transition-colors cursor-pointer"
